...
- **런(run)**: 웨이브 구분 없이 전체 분량의 오크가 텀 없이 계속 쏟아집니다.
  전부 막아내면 승리, 기지 체력이 0이 되면 패배.
- **타워는 런당 슬롯 제한**이 있습니다 — 기본은 1개만 설치 가능하고,
  더 설치하려면 업그레이드 트리에서 "타워 슬롯 확장" 노드를 구매해야 합니다.
- 런이 끝나면(승리든 패배든) **정수(essence)**를 얻습니다.
- 정수로 업그레이드 트리에서 영구 강화(데미지, 사거리, 공격속도, 시작 골드,
  기지 체력, 타워 슬롯 확장, 신규 타워 해금 등)를 구매할 수 있습니다.
  진행 상황은 `localStorage`에 저장되어 브라우저를 닫아도 유지됩니다.
